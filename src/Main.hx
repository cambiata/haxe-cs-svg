...
@:nativeGen class Main {
	...
	static public function main() {
			trace('Create a bitmap and render some graphic primitives...');
			var bitmap:cs.system.drawing.Bitmap = new cs.system.drawing.Bitmap(150, 400);
			...
			trace('Render a svg example to the bitmap...');
			var svg:svg.SvgDocument = svg.SvgDocument.FromSvg(svgExample);
			svg.Draw(gr);			
			gr.Save();
			trace('Save the bitmap as svgExample.png...');
			bitmap.Save('svgExample.png');
			trace('Done!');
	}
	
	static public var svgExample:String = 
 ...
 @:nativeGen class Mainx
{
	static public function main() 
	{
		...
		var btn = new Button();
		btn.Label = 'Create Clef.png';
		btn.add_Clicked(new cs.system.EventHandler(function(obj:Dynamic,e:cs.system.EventArgs) {
			var bitmap:cs.system.drawing.Bitmap = new cs.system.drawing.Bitmap(150, 400);
			var gr:cs.system.drawing.Graphics = cs.system.drawing.Graphics.FromImage(bitmap);
			gr.FillEllipse(cs.system.drawing.Brushes.Red, new cs.system.drawing.RectangleF(0, 0, 100, 100));
			var svg:svg.SvgDocument = svg.SvgDocument.FromSvg(ExSvg.clefG);
			svg.Draw(gr);			
			gr.Save();
			bitmap.Save('glefG.png');
		}));
		...
		var svgDrawing:SvgDrawing = new SvgDrawing();		
...
@:nativeGen class SvgDrawing extends gtk.DrawingArea {
	public function new() {
		super();
		this.SetSizeRequest(200, 400);
		this.svgExample = svg.SvgDocument.FromSvg(ExSvg.clefG);
	}
	
	var svgExample:svg.SvgDocument ;
	
	@:overload @:protected override function OnExposeEvent(e: gdk.EventExpose):Bool {
		
		var graphics:cs.system.drawing.Graphics = gtk.dotnet.Graphics.FromDrawable(e.Window);
		graphics.FillEllipse(cs.system.drawing.Brushes.Red, new cs.system.drawing.RectangleF(0, 0, 100, 100));
		graphics.DrawLine(new cs.system.drawing.Pen(cs.system.drawing.Color.Blue), 0, 0, 100, 100);
		
		this.svgExample.Draw(graphics);
...
/*
class SimpleList extends gtk.TreeView {
	public function new() {
		super();
		this.get_BeforeSignals().Clear();
	}
	
	@:overload @:protected override function BeforeSignals() {
